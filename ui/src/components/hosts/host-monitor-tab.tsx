...
export function HostMonitorTab({ hostId }: HostMonitorTabProps) {
  ...
  const { data: probeHistory = [] } = useQuery({
    ...
    queryFn: () => {
      const hours = parseInt(
        (TIME_RANGES[timeRange].duration / (60 * 60 * 1000)) as any
      )
  ...
  const groupedProbeHistory = {
    http: probeHistory.filter(
      (item: ServiceHistoryInfo) => item.service_monitor_type === 'HTTP'
    ),
    tcp: probeHistory.filter(
      (item: ServiceHistoryInfo) => item.service_monitor_type === 'TCP'
    ),
    icmp: probeHistory.filter(
      (item: ServiceHistoryInfo) => item.service_monitor_type === 'ICMP'
    ),
  }
