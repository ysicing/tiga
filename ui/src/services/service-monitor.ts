...
export const ServiceMonitorService = {
  ...
  async list(
    params?: ServiceMonitorListParams
  ): Promise<ServiceMonitorListResponse> {
  ...
  async get(id: string): Promise<ServiceMonitor> {
    const response = await apiClient.get<{ data: ServiceMonitor }>(
      `/vms/service-monitors/${id}`
    )
  ...
  async create(data: ServiceMonitorFormData): Promise<ServiceMonitor> {
    const response = await apiClient.post<{ data: ServiceMonitor }>(
      '/vms/service-monitors',
      data
    )
    return response.data
  },
  ...
  async update(
    id: string,
    data: Partial<ServiceMonitorFormData>
  ): Promise<ServiceMonitor> {
    const response = await apiClient.put<{ data: ServiceMonitor }>(
      `/vms/service-monitors/${id}`,
      data
    )
    return response.data
  ...
  async import(file: File): Promise<{ imported: number; failed: number }> {
    ...
    }>('/vms/service-monitors/import', formData)
    return response.data
  },
