...
const getStatusBadge = (status: string) => {
  switch (status) {
    case 'online':
      return <Badge className="bg-green-500"><IconCheck className="w-3 h-3 mr-1" /> 在线</Badge>
    case 'offline':
      return <Badge variant="secondary"><IconX className="w-3 h-3 mr-1" /> 离线</Badge>
    case 'pending':
      return <Badge variant="outline"><IconClock className="w-3 h-3 mr-1" /> 待检查</Badge>
    case 'error':
      return <Badge variant="destructive"><IconAlertCircle className="w-3 h-3 mr-1" /> 错误</Badge>
    default:
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{(error as any)?.message || '无法加载实例列表'}</CardDescription>
          </CardHeader>
        </Card>
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">数据库实例</h1>
          <p className="text-muted-foreground mt-2">管理 MySQL、PostgreSQL 和 Redis 数据库实例</p>
        </div>
              ...
              <CardContent>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{instance.type.toUpperCase()}</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="flex gap-2 mt-4">
                  <Button
                    ...
                    size="sm"
                    className="flex-1"
                    onClick={() => navigate(`/database/instances/${instance.id}`)}
                  >
