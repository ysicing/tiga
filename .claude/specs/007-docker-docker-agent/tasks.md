...
- [ ] **T007** [P] 定义DockerInstanceRepository接口
...
- [ ] **T008** 实现DockerInstanceRepository
  - 实现所有接口方法
  - 使用GORM查询构建器
  - 添加分页和排序支持
...
- [ ] **T023** 注册DockerService到gRPC Server
  - 在Agent启动时注册服务
  - 添加连接日志
...
- [ ] **T034** [P] 实现DockerInstanceHandler
...
- [ ] **T045** [P] 创建Docker实例列表页
  - 显示实例列表（名称、健康状态、容器数、镜像数）
  - 支持搜索和过滤
  - 添加创建实例按钮
  - 健康状态指示器（绿色/红色/灰色）
  - **文件**：`ui/src/pages/docker/instance-list-page.tsx`

- [ ] **T046** [P] 创建Docker实例详情页
  - 显示实例元数据
  - 三个Tab：容器、镜像、操作历史
  - 测试连接按钮
  - **文件**：`ui/src/pages/docker/instance-detail-page.tsx`

- [ ] **T047** [P] 创建Docker实例表单页
  - 创建/编辑表单（名称、描述、Agent选择、标签）
  - 表单验证
  - **文件**：`ui/src/pages/docker/instance-form-page.tsx`
...
- [ ] **T053** [P] 创建Docker审计日志页
  - 复用现有审计日志页面
  - 添加Docker资源类型过滤
  - 显示操作详情（JSON展开）
  - **文件**：`ui/src/pages/system/audit-logs-page.tsx`（扩展）

- [ ] **T054** [P] 创建Docker API客户端
  - 实现所有API调用函数
  - 使用fetch或axios
  - 添加错误处理
...
- [ ] **T055** 注册前端路由
  ...
  - 注册所有页面组件
  - 添加权限守卫
  - **文件**：`ui/src/App.tsx`、`ui/src/routes.tsx`
  - **依赖**：T045-T053

- [ ] **T056** 添加导航菜单
  - 在侧边栏添加Docker菜单项
  - 添加图标和子菜单
  - **文件**：`ui/src/layouts/main-layout.tsx`
  - **依赖**：T055
...
**Phase C完成标准**：
- [ ] AgentForwarder可以调用Agent gRPC
- [ ] 健康检查任务正常运行（60秒间隔）
- [ ] 审计日志正确记录所有操作

**Phase D完成标准**：
- [ ] 所有REST端点返回正确的JSON格式
- [ ] WebSocket终端可以连接并执行命令
- [ ] Swagger文档生成成功
