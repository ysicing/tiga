...
func (r *AuditRepository) List(ctx context.Context, f AuditFilter, page, pageSize int) ([]*models.MinIOAuditLog, int64, error) {
    ...
    if f.From != nil { q = q.Where("created_at >= ?", *f.From) }
    if f.To != nil { q = q.Where("created_at <= ?", *f.To) }
    if f.OperatorID != nil { q = q.Where("operator_id = ?", *f.OperatorID) }
    if f.Resource != nil && *f.Resource != "" { q = q.Where("resource_name = ?", *f.Resource) }

    var total int64
    if err := q.Count(&total).Error; err != nil { return nil, 0, err }
    if page > 0 && pageSize > 0 { q = q.Offset((page-1)*pageSize).Limit(pageSize) }
    var items []*models.MinIOAuditLog
    if err := q.Order("created_at DESC").Find(&items).Error; err != nil { return nil, 0, err }
