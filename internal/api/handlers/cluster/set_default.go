...
// @Router /api/v1/k8s/clusters/{id}/set-default [post]
// @Security Bearer
func (h *ClusterHandler) SetDefault(c *gin.Context) {
	...
	// Set as default
	if err := h.clusterRepo.SetDefault(ctx, clusterID); err != nil {
		logrus.Errorf("Failed to set default cluster %s: %v", clusterID, err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"code":    http.StatusInternalServerError,
			"message": "Failed to set default cluster",
		})
		return
	}
